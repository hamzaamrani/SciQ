...
Since `L` and `R` can be any left or right parenthesis, and every matrices must have the same number of columns, to render a system of equation one can write something like `{[(root n x)/(x) <= 4], [x^2=e^x]:}`, that will be rendered as $\left\{\begin{matrix}\frac{\sqrt[n]{x}}{x} \le 4 \\ x^{2} = e^{x}\end{matrix}\right.$.  
...
that will be rendered as $\left[\begin{matrix}\frac{\sqrt[n]{x}}{x} \le 4  &  \int x dx \\ x^{2} = e^{x}  &  \lim_{x \to \infty} \frac{1}{x^2} \end{matrix}\right]$