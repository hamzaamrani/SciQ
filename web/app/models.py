...
user_expression = db.Table(
    ...
    db.Column(
        "expression_id",
        db.Integer,
        db.ForeignKey("expression.id", ondelete="cascade"),
    ),
...
class User(db.Model):
    ...
    expressions = db.relationship(
        "Expression",
        secondary=user_expression,
        backref="users",
        lazy="dynamic",
    )
...
def delete_expression_orphans(session, ctx):
    session.query(Expression).filter(~Expression.users.any()).delete(
